...
@interface MKBPMEnergyReportController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBPMEnergyReportModel *dataModel;

@end

@implementation MKBPMEnergyReportController

- (void)dealloc {
    NSLog(@"MKBPMEnergyReportController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Energy Report  Setting";
    [self.rightButton setImage:LOADICON(@"MKBlePlugMax", @"MKBPMEnergyReportController", @"bpm_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBPMEnergyReportModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBPMEnergyReportModel alloc] init];
    }
    return _dataModel;
}
